...
let MaterialsService = class MaterialsService {
    ...
    async ensureCourse(courseId, actor) {
    ...
    async ensureOwnedCourse(courseId, actor) {
    ...
    async create(courseId, dto, actor, file) {
    ...
    async remove(courseId, id, actor) {
        await this.ensureOwnedCourse(courseId, actor);
    ...
    async update(courseId, id, dto, actor, file) {
        await this.ensureOwnedCourse(courseId, actor);
        const material = await this.prisma.material.findFirst({ where: { id, course_id: courseId } });
        if (!material)
            throw new NotFoundException('Materi tidak ditemukan');
