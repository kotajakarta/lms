...
let StudentService = class StudentService {
    ...
    async getOverview(userId) {
        ...
        const courseIds = enrollments.map((item) => item.course_id).filter((id) => id !== null);
    ...
    async createReply(userId, dto) {
        ...
        if (!topic)
            throw new NotFoundException('Topik diskusi tidak ditemukan');
        return this.prisma.balasanDiskusi.create({
            data: { topic_id: dto.topic_id, user_id: userId, isi: dto.isi },
            include: { user: { select: { id: true, nama: true, foto: true, role: true } } },
        });
    }
    async updateMaterialProgress(userId, materialId, completed) {
        ...
        if (enrollment)
            await this.prisma.enrollment.update({ where: { id: enrollment.id }, data: { progress, last_accessed: new Date() } });
        else if (isPublic)
            await this.prisma.enrollment.create({ data: { user_id: userId, course_id: material.course_id, status: 'active', progress, last_accessed: new Date() } });
